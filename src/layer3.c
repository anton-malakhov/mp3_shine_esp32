...
int shine_check_config(int freq, int bitr)
{
  ...
  if (samplerate_index < 0) {
    ESP_LOGE(TAG, "Sample rate %d is unknown", freq);
    return -1;
  }
  ...
  if (bitrate_index < 0) {
    ESP_LOGE(TAG, "Bit rate %d is not supported for sample rate %d", bitr, freq);
    return -1;
  }
  return mpeg_version;
}
...
shine_global_config *shine_initialise(shine_config_t *pub_config)
{
  ...
  ESP_LOGD(TAG, "shine_global_config struct: %d", sizeof(shine_global_config));
  config = heap_caps_malloc(sizeof(shine_global_config), MALLOC_CAP_8BIT|MALLOC_CAP_INTERNAL);
  if (config == NULL)
    return config;
  ESP_LOGD(TAG, "l3loop struct: %d", sizeof(l3loop_t));
  p = config->l3loop = heap_caps_malloc(sizeof(l3loop_t), MALLOC_CAP_8BIT|MALLOC_CAP_INTERNAL);
  if(p == NULL) return p;
  ESP_LOGD(TAG, "l3_enc & mdct_freq each: %d", sizeof(int32_t)*GRANULE_SIZE*MAX_GRANULES*MAX_CHANNELS);
  for (x = 0; x < MAX_CHANNELS; x++) {
      for (y = 0; y < MAX_GRANULES; y++) {
        // 2 * 2 * 576 each        
        p = config->l3_enc[x][y] = heap_caps_malloc(sizeof(int32_t)*GRANULE_SIZE, MALLOC_CAP_32BIT|MALLOC_CAP_INTERNAL); //Significant performance hit in IRAM
        if(p == NULL) return p;
        p = config->mdct_freq[x][y] = heap_caps_malloc(sizeof(int32_t)*GRANULE_SIZE, MALLOC_CAP_32BIT); //OK 1%
        if(p == NULL) return p;
      }
  }
  ESP_LOGD(TAG, "xrsq & xrabs each: %d", sizeof(int32_t)*GRANULE_SIZE);
  p = config->l3loop->xrsq = heap_caps_malloc(sizeof(int32_t)*GRANULE_SIZE, MALLOC_CAP_32BIT); //OK 0.5%
  if(p == NULL) return p;
  p = config->l3loop->xrabs = heap_caps_malloc(sizeof(int32_t)*GRANULE_SIZE, MALLOC_CAP_32BIT); //OK 0.5%
  if(p == NULL) return p;
...
static SHINE_IRAM unsigned char *shine_encode_buffer_internal(shine_global_config *config, int *written, int stride)
{
