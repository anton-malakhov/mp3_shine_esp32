 ...
 */
void SHINE_IRAM shine_window_filter_subband(int16_t **buffer, int32_t s[SBLIMIT], int ch, shine_global_config *config, int stride)
{
  int32_t y[64];
  int i,j;
  int16_t *ptr = *buffer;
  ...
  *buffer = ptr;
  ...
  for (i=64; i--; ) {
    ...
    for(j=1; j<8; j++)
      muladd(s_value, s_value_lo, config->subband.x[ch][(config->subband.off[ch] + i + (j<<6)) & (HAN_SIZE-1)], shine_enwindow[i + (j<<6)]);
  ...
  for (i=SBLIMIT; i--; ) {
    ...
    for (j=63; j; j-=9) {
      ...
      muladd(s_value, s_value_lo, config->subband.fl[i][j-8], y[j-8]);
      muladd(s_value, s_value_lo, config->subband.fl[i][j-9], y[j-9]);
    }
    mulz(s_value, s_value_lo);
    s[i] = s_value;
  }
