...
int32_t sqrt2int(double r)  // improves by 0.4%
{
    union {
      float f;
      uint32_t i;
    ...
    float y = rr.f*0.5;
    x.i = (0xbe6f0000 - rr.i) >> 1;
    x.f = (1.5f*x.f) - (x.f*x.f)*(x.f*y);
    if(rr.f>101123)
      x.f = (1.5f*x.f) - (x.f*x.f)*(x.f*y);
    int32_t is = x.f*rr.f + 0.5f;
    return is + (((int32_t)r /*from double!*/ - is*is)>>31);
}
 ...
 * of scalefacs.
 */
int SHINE_IRAM shine_inner_loop(int ix[GRANULE_SIZE],
...
int SHINE_IRAM shine_outer_loop( int max_bits,
 ...
 * ------------------
 */
void SHINE_IRAM shine_iteration_loop(shine_global_config *config)
{
 ...
 */
void SHINE_IRAM calc_scfsi( shine_psy_xmin_t *l3_xmin, int ch, int gr,
                 shine_global_config *config )
{
 ...
 */
void SHINE_IRAM calc_xmin(gr_info *cod_info,
               shine_psy_xmin_t *l3_xmin,
               int gr, int ch )
{
  int sfb;
 ...
 */
int SHINE_IRAM quantize(int ix[GRANULE_SIZE], int stepsize, shine_global_config *config )
{
  int i, max, ln;
  int32_t scalei;
  float scale, dbl;
  ...
  if((mulr(config->l3loop->xrmax,scalei)) > 165140) /* 8192**(4/3) */
    ...
    for(i=0, max=0;i<GRANULE_SIZE;i++)
    {
      ...
      else
      {
        ...
        ix[i] = sqrt2int(sqrt(dbl)*dbl); /* dbl**(3/4) */
      }
